...
{   char uhrzeit[32] = "00:00:00 DE";
    char zone[4];
    int _hrs = hrs;                             // Save hrs in another variable to not overwrite the time
    
    if(mode==0){                                // Check for clockmode, fill zone string accordingly
      (void) sprintf(zone, "DE");               
    } else {
      (void) sprintf(zone, "US");
        _hrs = hrs - 6;                         // If in US time mode subtract 6 hrs and check for underflow
        if(_hrs < 0) {
          _hrs = 24 + _hrs;
        }
    }
    
    (void) sprintf(uhrzeit, "%02d:%02d:%02d %s", _hrs, mins, secs, zone );
    writeLine(uhrzeit, 0);
}
