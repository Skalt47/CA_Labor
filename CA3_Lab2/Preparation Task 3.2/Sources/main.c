...
void main(void) 
{
    ...
    static int  seconds = 0;         // count seconds in software
    char         buf[7];             // “–00000\0” needs 7 bytes

    for (;;) {                       // Endless loop
        if (clockEvent) {
            clockEvent = 0;          // clear the flag

            seconds++;               // advance your software counter

            decToASCII_Wrapper(buf, seconds);
